...
class EmbedderApiBuilder {
 public:
  void SetProject(flutter::FlutterProjectBundle project);
  void SetEGLManager(egl::Manager manager);
  void SetCompositor(flutter::Compositor compositor);

  void SetPlatformMessageCallback(
      std::function<const FlutterDesktopMessage&> callback);
  void SetVSyncCallback(std::function<int64_t> callback);
  ...
  void SetChannelUpdateCallback(fml::closure callback);

  std::unique_ptr<EmbedderApi> Build();
};
