...
class Manager {
 public:
  ...
  // Resizes the surface to the desired size.
  //
  // If the dimensions have changed, this destroys the original surface and
  // creates a new one.
  //
  // This binds the surface to the current thread.
  //
  // HWND is the window backing the surface. Width and height are the surface's
  // desired physical pixel dimensions.
  //
  // Returns null on failure or if the dimensions have not changed.
  virtual std::unique_ptr<WindowSurface> ResizeWindowSurface(
      WindowSurface* original_surface,
      HWND hwnd,
      size_t width,
      size_t height);
