...
void UpdateVsync(const FlutterWindowsEngine& engine,
  ...
  if (engine.running()) {
    engine.PostRasterThreadTask(
        [surface, needs_vsync]() { surface->SetVSyncEnabled(needs_vsync); });
  } else {
...
bool FlutterWindowsView::OnEmptyFrameGenerated() {
  ...
  if (!ResizeRenderSurface(resize_target_height_, resize_target_height_)) {
    return false;
  }
...
bool FlutterWindowsView::OnWindowSizeChanged(size_t width, size_t height) {
  ...
  if (!engine_->egl_manager() || !surface_) {
...
bool FlutterWindowsView::ResizeRenderSurface(size_t width, size_t height) {
  ...
  if (!surface_->Destroy()) {
    FML_LOG(ERROR) << "View resize failed to destroy the existing surface";
    return false;
  }
  ...
  std::unique_ptr<egl::WindowSurface> resized_surface =
      engine_->egl_manager()->CreateWindowSurface(GetWindowHandle(), width,
                                                  height);
  if (!resized_surface) {
    FML_LOG(ERROR) << "View resize failed to create a new surface";
    return false;
  }
  ...
  if (!resized_surface->SetVSyncEnabled(existing_vsync)) {
    ...
    FML_LOG(ERROR) << "View resize failed to set new surface vsync";
  }
...
egl::WindowSurface* FlutterWindowsView::GetSurface() const {
  return surface_.get();
}
