...
bool CompositorOpenGL::Present(int64_t view_id,
  ...
  if (!engine_->view(view_id)->OnFrameGenerated(width, height)) {
    return false;
  }

  if (!engine_->surface_manager()->MakeSurfaceCurrent(view_id)) {
    return false;
  }
  ...
  return engine_->view(view_id)->SwapBuffers();
}

bool CompositorOpenGL::Initialize() {
  FML_DCHECK(!is_initialized_);

  if (!engine_->surface_manager()->MakeRenderContextCurrent()) {
    return false;
  }
...
bool CompositorOpenGL::ClearSurface(int64_t view_id) {
  FML_DCHECK(is_initialized_);

  // Resize the surface if needed.
  engine_->view(view_id)->OnEmptyFrameGenerated();
