...
TEST_F(CompositorOpenGLTest, CreateBackingStore) {
  ...
  EXPECT_CALL(*surface_manager(), MakeRenderContextCurrent)
...
TEST_F(CompositorOpenGLTest, Present) {
  ...
  EXPECT_CALL(*surface_manager(), MakeSurfaceCurrent).WillOnce(Return(true));
  EXPECT_CALL(*view(), SwapBuffers).WillOnce(Return(true));
  EXPECT_TRUE(compositor.Present(&layer_ptr, 1, kImplicitViewId));
...
TEST_F(CompositorOpenGLTest, HeadlessPresentIgnored) {
  ...
  EXPECT_FALSE(compositor.Present(&layer_ptr, 1, kImplicitViewId));
